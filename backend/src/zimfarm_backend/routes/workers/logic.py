...
async def check_in_worker(
    ...
    db_check_in_worker(
        ...
        offliners=[offliner.value for offliner in worker_checkin.offliners],
        platforms=(
            worker_checkin.platforms.model_dump(  # pyright: ignore[reportUnknownMemberType, reportUnknownArgumentType]
                mode="json"
            )
            if worker_checkin.platforms  # pyright: ignore[reportUnknownMemberType]
            else {}
        ),
