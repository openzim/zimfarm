...
from zimfarm_backend.common.schemas.orms import (
    ConfigWithOnlyTaskNameAndResourcesSchema,
    NameOnlySchema,
...
from zimfarm_backend.routes import check_user_permission
...
from zimfarm_backend.routes.utils import remove_secrets_from_payload
...
def create_request_task(
    ...
    if not check_user_permission(current_user, namespace="task", name="request"):
...
def get_requested_tasks(
    ...
    resource_keys: dict[str, int] = {}
    if requested_task_schema.matching_cpu is not None:
        resource_keys["cpu"] = requested_task_schema.matching_cpu
    if requested_task_schema.matching_memory is not None:
        resource_keys["memory"] = requested_task_schema.matching_memory
    if requested_task_schema.matching_disk is not None:
        resource_keys["disk"] = requested_task_schema.matching_disk
    results = db_get_requested_tasks(
        session,
        worker_name=requested_task_schema.worker,
        skip=requested_task_schema.skip,
        limit=requested_task_schema.limit,
        matching_offliners=(
...
def get_requested_task(
    ...
    hide_secrets: Annotated[bool, Query(default=False)],
    session: Annotated[OrmSession, Depends(gen_dbsession)],
    current_user: Annotated[User | None, Depends(get_current_user_or_none)],
) -> JSONResponse:
    ...
    payload = requested_task.model_dump(mode="json")
    payload["rank"] = compute_requested_task_rank(session, requested_task_id)
    ...
    if hide_secrets:
        remove_secrets_from_payload(payload)
...
def update_requested_task(
    ...
    if not check_user_permission(current_user, namespace="task", name="update"):
...
def delete_requested_task(
    ...
    if not check_user_permission(current_user, namespace="task", name="unrequest"):
