...
def request_task(
    ...
    *,
    ...
    requested_task = RequestedTask(
        ...
        priority=priority,
        config=expanded_config(schedule.config),
        upload={
            "zim": {
...
def get_requested_tasks(
    ...
    *,
    ...
    resource_keys: dict[str, int] | None = None,
) -> RequestedTaskListResult:
    """Get a list of requested tasks."""
    query = (
        select(
    ...
    if priority is not None:
        query = query.where(RequestedTask.priority >= priority)
    if worker_name is not None:
        query = query.where(
            or_(
                RequestedTask.worker.is_(None),
                Worker.name == worker_name,
            )
        )
    if resource_keys is not None:
        for res_key in resource_keys:
            ...
            query = query.where(
                RequestedTask.config["resources"][res_key].astext.cast(BigInteger)
                <= resource_keys[key]
            )

    if matching_offliners is not None:
        query = query.where(
            RequestedTask.config["task_name"].astext.in_(matching_offliners)
        )
...
def compute_task_eta(session: OrmSession, task: Task) -> dict[str, Any]:
    ...
    duration = get_schedule_duration(
        session, schedule_name=task.schedule.name, worker=task.worker
    )
...
def get_tasks_doable_by_worker(
    ...
    query = select(RequestedTask).where(
        ...
        <= worker.disk,
        RequestedTask.config["task_name"].astext.in_(worker.offliners),
    )
    if worker.selfish:
    ...
    return sorted(
        [
            RequestedTaskWithDuration(
                ...
                duration=get_schedule_duration(
                    session, schedule_name=task.schedule.name, worker=worker
                ),
...
def _create_requested_task_full_schema(
    ...
    return RequestedTaskFullSchema(
        ...
        config=ConfigWithOnlyTaskNameAndResourcesSchema(
            task_name=requested_task.config["task_name"],
            resources=requested_task.config["resources"],
        ),
        ...
        schedule=NameOnlySchema(name=requested_task.schedule.name),
    )
