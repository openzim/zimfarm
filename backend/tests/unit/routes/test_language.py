...
def test_get_languages_custom_pagination(client: TestClient):
    ...
    response = client.get("/languages?skip=10&limit=20")
    assert response.status_code == HTTPStatus.OK
    data = response.json()
    assert "meta" in data
    assert "items" in data
...
def test_get_languages_invalid_pagination(client: TestClient):
    ...
    response = client.get("/languages?skip=-1")
    assert response.status_code == HTTPStatus.UNPROCESSABLE_ENTITY

    # Test with negative limit
    response = client.get("/languages?limit=-1")
    assert response.status_code == HTTPStatus.UNPROCESSABLE_ENTITY

    # Test with limit exceeding maximum
    response = client.get("/languages?limit=501")
    assert response.status_code == HTTPStatus.UNPROCESSABLE_ENTITY
