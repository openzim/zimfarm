...
def test_verify_kiwix_access_token_with_2fa_enabled_and_valid_amr(
    monkeypatch: pytest.MonkeyPatch,
):
    ...
    monkeypatch.setattr(token_module, "KIWIX_LOGIN_REQUIRE_2FA", True)
    ...
    with (
        ...
        assert "password" in result["amr"]
        assert "totp" in result["amr"]


def test_verify_kiwix_access_token_with_2fa_enabled_only_first_factor(
    monkeypatch: pytest.MonkeyPatch,
):
...
def test_verify_kiwix_access_token_with_2fa_disabled_only_first_factor(
    ...
    monkeypatch.setattr(token_module, "KIWIX_LOGIN_REQUIRE_2FA", False)
