...
export function buildDockerCommand(name: string, config: ExpandedScheduleConfig) {
  ...
  const mem_params = [
    ...
    "0",
    "--memory",
    config.resources.memory.toString(),
  ];
  ...
  const cpu_params = ["--cpu-shares", (config.resources.cpu * constants.DEFAULT_CPU_SHARE).toString()];
  ...
  const args = docker_base
    .concat([imageHuman(config)])
...
export function buildScheduleDuration(duration: ScheduleDuration | null): SingleScheduleDuration | MultipleScheduleDuration | null {
  if (!duration) return null;
  function single_duration(value: number, worker: string, on: string): SingleScheduleDuration {
    return { single: true, value: value, worker: worker, on: on, minValue: null, maxValue: null, minWorkers: null, maxWorkers: null };
  }

  function multipleDurations(minValue: number, maxValue: number, workers: Record<string, WorkerScheduleDuration>): MultipleScheduleDuration {
  ...
  const minWorker = workersArray.reduce((min, worker) => (worker.value < min.value ? worker : min), workersArray[0]);
  const maxWorker = workersArray.reduce((max, worker) => (worker.value > max.value ? worker : max), workersArray[0]);
