...
class Thread(threading.Thread):
    ...
    def __init__(
        self, socket: socket.socket, address: (str, int), host_key: paramiko.RSAKey
    ):
        super().__init__()
        self.daemon = True
        self.socket = socket
    ...
    def run(self):
        self.logger.info(
            "Received incoming connection -- {}:{}".format(
        ...
        try:
            with paramiko.Transport(self.socket) as transport:
                ...
                transport.set_subsystem_handler(
                    name="sftp", handler=paramiko.SFTPServer, sftp_si=sftp.Handler
                )
