...
class Blueprint(flask.Blueprint):
    def __init__(self):
        ...
        self.add_url_rule(
            "/<string:user>", "delete_user", user.delete, methods=["DELETE"]
        )

        self.add_url_rule(
            "/<string:user>/keys", "list_ssh_keys", keys.list, methods=["GET"]
        )
        self.add_url_rule(
            "/<string:user>/keys", "add_ssh_keys", keys.add, methods=["POST"]
        )
        self.add_url_rule(
            "/<string:user>/keys/<string:fingerprint>",
            "delete_ssh_keys",
            keys.delete,
            methods=["DELETE"],
        )
