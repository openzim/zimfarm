...
class TestScheduleOfflinerGet:
    def test_offliner_mwoffliner(self, client, access_token, schedule):
        """Test get mwoffliner"""

        schedule_id = schedule['_id']
        url = '/api/schedules/{schedule}/offliner'.format(schedule=str(schedule_id))
        response = client.get(url, headers={'Authorization': access_token})
        assert response.status_code == 200
        assert schedule['offliner'] == response.get_json()

        schedule_name = schedule['name']
        url = '/api/schedules/{schedule}/offliner'.format(schedule=str(schedule_name))
        response = client.get(url, headers={'Authorization': access_token})
        assert response.status_code == 200
        assert schedule['offliner'] == response.get_json()
...
class TestScheduleOfflinerMWOfflinerUpdate:
    def test_update_full(self, client, access_token, make_offliner_mwoffliner, make_schedule):
        """Test update offliner with new mwoffliner config"""

        schedule = make_schedule('name', 'language', 'wikipedia')
        url = '/api/schedules/{schedule}/offliner'.format(schedule=str(schedule['_id']))
        offliner = make_offliner_mwoffliner(sub_domain='bm', admin_email='admin@example.com')
    ...
    def test_bad_content_type(self, client, access_token, schedule, make_offliner_mwoffliner):
    ...
    def test_patch_no_access_token(self, client, schedule, make_offliner_mwoffliner):
    ...
    def test_patch_bad_schedule_id_or_name(self, client, access_token, make_offliner_mwoffliner):
